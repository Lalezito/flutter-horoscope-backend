...
class WeeklyController {
  ...
  async storeWeeklyHoroscopes(horoscopes) {
    ...
    for (const horoscope of horoscopes) {
      try {
        const { sign, language_code, content, week_start, week_end } =
          horoscope;
        ...
        console.error(
          `Error storing weekly horoscope for ${horoscope.sign}:`,
          error
        );
        results.errors++;
        results.details.push({
      ...
      if (!force && !fill_missing) {
        ...
        if (parseInt(checkResult.rows[0].count) > 0) {
          return res.status(400).json({
            ...
            message:
              "Use force=true to regenerate or fill_missing=true to complete",
            week: `${weekStart} to ${weekEnd}`,
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You are a professional astrologer with deep knowledge of zodiac signs and planetary influences. You provide insightful, positive, and actionable weekly horoscopes.",
                },
                {
                  role: "user",
                  content: prompt,
